...
(function () {
    ...
    (function selfUpdate() {
        var re = /(designer|billionthemler)\/adminhtml_theme/;
        if (re.test(window.location.href)) {
            window.location.href = window.location.href.replace(re, 'admin/billionthemler_theme');
        }
    })();
    ...
    DataProvider.getInfo = function getInfo() {
        return {
            ...
            canDuplicateTemplatesConstructors: {
                'HomeTemplate': true,
                'PageTemplate': true,
                'ProductOverviewTemplate': true,
                'ProductsTemplate': true,
                'Template404': true
            }
        };
    };
    ...
    DataProvider.reloadTemplatesInfo = function reloadTemplatesInfo(callback) {
        ...
        $.ajax(_ajax({
            type: 'post',
            url: 'reloadTemplatesInfo',
            dataType: 'json',
            data: {}
        })).done(function reloadTemplateInfoSuccess(response, status, xhr) {
            var error = DataProvider.validateResponse(xhr);
            if (!error) {
                Object.keys(response).forEach(function (key) {
                    window.themeDesigner[key] = response[key];
                });
            }
            callback(error);
